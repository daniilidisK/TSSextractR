...
shinyUI(navbarPage("TSS ExtractR", collapsible = T, theme = shinytheme("cerulean"),
                          ...
                          fluidPage(
                            ...
                            sidebarLayout(
                              sidebarPanel(
                                ...
                                selectInput("refgenome", "Select Reference Genome", c("E. Coli K-12 MG1655", "H. Pylori")),
                 ...
                 tabPanel("TSS Annotation", fluidPage(
                   sidebarLayout(
                     sidebarPanel(
                       ...
                       textOutput("dfckecker2"),
                       div(style = "margin-top: 30px"),
                       fileInput("annotationFile", "Or you can upload Existing and Clustered CSV File", multiple = FALSE,
                                 accept = c("text/csv", "text/comma-separated-values,text/plain", ".csv")),
                       checkboxInput("header2", "Header", TRUE),
                       radioButtons("sep2", "Separator", choices = c(Comma = ",", Semicolon = ";",
                                                                     Tab = "\t"), selected = ","),
                       radioButtons("quote2", "Quote", choices = c(None = "", "Double Quote" = '"',
                                                                   "Single Quote" = "'"), selected = '"'),
                       hr(style = "border-color: lightgrey"),
                       
                       fileInput("genomeFile", "Gene Annotation File", multiple = FALSE),
                       checkboxInput("geneHeader", "Header", FALSE),
                       radioButtons("geneSep", "Separator", choices = c(Comma = ",", Semicolon = ";",
                                                                     Tab = "\t"), selected = "\t")),
                     
                     mainPanel(h1(textOutput("clTitle2")), plotOutput("ann1", click = "plot_click"), dataTableOutput("annotated"))
                   )
                 )),
                 ...
                 tabPanel("Info", fluidPage(
                   div(tags$img(src="http://www.microrna.gr/dianalab/wp-content/uploads/2019/04/diana_banner.png"), style="text-align:center"),
                   h2(strong("TSS ExtractR"), align = "center"),
                   hr(style = "border-color: lightgrey"),
                   p("Daniilidis Konstantinos", align = "center", style = "font-size:17px")
                 ))
                ))
